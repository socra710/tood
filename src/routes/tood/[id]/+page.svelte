<script lang="ts">
  ...
  function loadKakaoMapScript(callback) {
    ...
    if (!document.getElementById(scriptId)) {
      ...
      script.id = scriptId;
      script.src = "https://dapi.kakao.com/v2/maps/sdk.js?appkey=38f3cb332825f04c4c29380a8fd7359c&autoload=false";
  ...
  $: if (!loading && buffet && mapContainer) {
    loadKakaoMapScript(() => {
      if (mapContainer && window.kakao && window.kakao.maps && buffet.lat && buffet.lng) {
        ...
        kakao.maps.load(() => {
          ...
          new window.kakao.maps.Marker({
            map,
            position: new window.kakao.maps.LatLng(Number(buffet.lat), Number(buffet.lng)),
          });
        });
  ...
  <main>
    ...
    {:else if buffet}
      ...
      <div class="buffet-info">
        <p><strong>지역:</strong> {buffet.region ? `${buffet.region} ` : ""}{buffet.location}</p>
